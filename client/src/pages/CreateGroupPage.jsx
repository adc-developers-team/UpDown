...
const CreateGroupPage = () => {
  ...
  const handleCreate = async () => {
    ...
    try {
      const { data } = await axios.post('https://updown-hms5.onrender.com/api/groups', { name: groupName, members: selected }, config);
  ...
  return (
    <div className="min-h-screen bg-chat-bg text-white">
      ...
      <div className="p-4 space-y-4">
        <input type="text" placeholder="Group name" value={groupName} onChange={e => setGroupName(e.target.value)} className="w-full bg-gray-800 rounded-full px-4 py-2 outline-none text-white placeholder-gray-400" />
        ...
        <button onClick={handleCreate} className="w-full bg-light-blue py-2 rounded-full font-semibold hover:bg-blue-600 transition">Create Group</button>
      </div>
    </div>
  );
};
