...
import {
  FiArrowLeft, FiCamera, FiCheck, FiPlus, FiX, FiSearch,
  FiChevronRight, FiChevronLeft, FiUsers, FiLock, FiGlobe,
...
const CreateGroupPage = () => {
  ...
  const [whoCanEdit, setWhoCanEdit] = useState('admins');
  ...
  const filteredFriends = friends.filter(f =>
    (f.fullName || f.username || '').toLowerCase().includes(searchMember.toLowerCase())
  );
    ...
    <div className="min-h-screen bg-chat-bg text-white pb-20">
      ...
      <header className="h-16 sm:h-[72px] flex items-center gap-4 px-4 bg-dark-blue border-b border-gray-700/50 sticky top-0 z-20">
        ...
        <h2 className="font-semibold text-lg">
          {step === 1 ? 'New Group' : step === 2 ? 'Add Members' : step === 3 ? 'Group Settings' : 'Group Created'}
        </h2>
        {step < 3 && (
          <div className="ml-auto flex items-center gap-1 text-sm text-text-secondary">
            <span className={`w-2 h-2 rounded-full ${step >= 1 ? 'bg-primary' : 'bg-gray-600'}`} />
            <span className={`w-2 h-2 rounded-full ${step >= 2 ? 'bg-primary' : 'bg-gray-600'}`} />
            <span className={`w-2 h-2 rounded-full ${step >= 3 ? 'bg-primary' : 'bg-gray-600'}`} />
          </div>
        )}
      </header>
          ...
          <div className="space-y-6 animate-fade-in">
                ...
                <div className="w-24 h-24 rounded-full bg-primary/10 flex items-center justify-center overflow-hidden border-2 border-dashed border-gray-600 group-hover:border-primary transition">
                  {groupImage ? (
                    ...
                    <div className="text-center">
                      <FiCamera size={28} className="mx-auto text-text-muted group-hover:text-primary transition" />
            ...
            <div>
              <label className="block text-sm font-medium text-text-secondary mb-1.5">
                Group Name <span className="text-xs text-text-muted">({groupName.length}/{MAX_NAME})</span>
              </label>
              <input
                type="text"
            ...
            <div>
              <label className="block text-sm font-medium text-text-secondary mb-1.5">
                Description <span className="text-xs text-text-muted">Optional ({groupDesc.length}/{MAX_DESC})</span>
              </label>
              <textarea
              ...
              <div className="space-y-2">
                ...
                >
                  ...
                  <div className="text-left">
                    ...
                    <p className="text-xs text-text-secondary">Only invited members can join</p>
                  </div>
                  {privacy === 'private' && <FiCheck size={18} className="ml-auto text-primary" />}
                </button>
                <button
                ...
                >
                  ...
                  <div className="text-left">
                    ...
                    <p className="text-xs text-text-secondary">Anyone can find and request to join</p>
                  </div>
                  {privacy === 'public' && <FiCheck size={18} className="ml-auto text-primary" />}
                </button>
              </div>
            </div>
        ...
        {step === 2 && (
          <div className="space-y-4 animate-fade-in">
            ...
            {selectedFriends.length > 0 && (
              <div className="flex flex-wrap gap-2 p-3 bg-surface rounded-xl border border-border-light">
                {selectedFriends.map(f => (
                  <span key={f._id} className="flex items-center gap-1 bg-primary/10 text-primary text-xs px-2.5 py-1 rounded-full">
                    {f.fullName || f.username}
                    <button onClick={() => toggleMember(f._id)} className="hover:text-danger"><FiX size={12} /></button>
                  </span>
                ))}
              </div>
            )}
        ...
        {step === 3 && (
          <div className="space-y-4 animate-fade-in">
            <div className="bg-surface rounded-xl border border-border-light p-4 space-y-3">
              <h3 className="font-medium text-sm">Group Permissions</h3>
              ...
              <div className="flex items-center justify-between py-1">
                <span className="text-sm">Enable Invite Link</span>
                <button onClick={() => setInviteLink(!inviteLink)} className={`relative inline-flex h-6 w-10 items-center rounded-full transition ${inviteLink ? 'bg-primary' : 'bg-gray-600'}`}>
                  <span className={`inline-block h-4 w-4 transform rounded-full bg-white transition ${inviteLink ? 'translate-x-5' : 'translate-x-1'}`} />
                </button>
              </div>

              <div className="flex items-center justify-between py-1">
                <span className="text-sm">Show message history to new members</span>
                <button onClick={() => setShowHistory(!showHistory)} className={`relative inline-flex h-6 w-10 items-center rounded-full transition ${showHistory ? 'bg-primary' : 'bg-gray-600'}`}>
                  <span className={`inline-block h-4 w-4 transform rounded-full bg-white transition ${showHistory ? 'translate-x-5' : 'translate-x-1'}`} />
                </button>
              </div>
            </div>
        ...
        {step === 4 && (
          <div className="flex flex-col items-center justify-center py-16 space-y-6 animate-fade-in">
            ...
            <div className="text-center space-y-2">
              ...
              <p className="text-text-secondary">Your group is ready to use.</p>
            </div>
