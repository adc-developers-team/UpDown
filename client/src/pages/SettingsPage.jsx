  ...
  return (
    <div className="min-h-screen bg-chat-bg text-white">
      ...
      <div className="px-4 py-6 space-y-6 max-w-2xl mx-auto">
        ...
        {isInstallable && <button onClick={installApp} className="w-full flex items-center gap-4 bg-accent hover:bg-accent-hover text-black font-semibold p-5 rounded-2xl transition-all"><FiDownload size={20} /> Install UpDown App</button>}
        ...
        <Section title="Blocked Users" icon={<FiLock size={20} className="text-accent" />} show={showBlocked} setShow={setShowBlocked}>
          {blockedUsers.length === 0 ? <p className="text-sm text-text-muted py-2">No blocked users</p> : blockedUsers.map(u => (
            <div key={u._id} className="flex items-center gap-3 p-2 bg-bg-input rounded-xl">
              <div className="w-8 h-8 rounded-full bg-accent/20 flex items-center justify-center overflow-hidden">{u.profilePic ? <img src={u.profilePic} className="w-full h-full object-cover" alt="" /> : <span className="text-xs font-bold text-accent">{u.fullName?.[0] || u.username?.[0]?.toUpperCase()}</span>}</div>
        ...
        <Section title="Active Sessions" icon={<FiMonitor size={20} className="text-accent" />} show={showSessions} setShow={setShowSessions}>
          {sessions.length === 0 ? <p className="text-sm text-text-muted py-2">No active sessions</p> : sessions.map((session, i) => (
      ...
      {deleteConfirm && (
        <div className="fixed inset-0 z-50 bg-black/60 flex items-center justify-center p-4">
          <div className="bg-sidebar-bg rounded-2xl p-6 max-w-sm w-full space-y-4">
            ...
            <div className="flex gap-3"><button onClick={() => setDeleteConfirm(false)} className="flex-1 py-2 bg-gray-700 rounded-full text-sm">Cancel</button><button onClick={handleDeleteAccount} className="flex-1 py-2 bg-red-600 rounded-full text-sm font-semibold">Delete Forever</button></div>
          </div>
        </div>
      )}
    </div>
...
const Section = ({ title, icon, show, setShow, children }) => (
  <div className="bg-sidebar-bg rounded-2xl border border-gray-700 overflow-hidden">
    <button onClick={() => setShow(!show)} className="w-full flex items-center gap-3 p-5 hover:bg-gray-800 transition">{icon}<span className="flex-1 text-left font-medium">{title}</span><span className="text-sm text-text-secondary">{show ? '▲' : '▼'}</span></button>
...
const ToggleRow = ({ icon, title, subtitle, checked, onChange }) => (
  <div className="bg-sidebar-bg rounded-2xl p-5 border border-gray-700 flex items-center justify-between">
    <div className="flex items-center gap-3"><div className={`w-10 h-10 rounded-full flex items-center justify-center ${checked ? 'bg-yellow-400/10 text-yellow-400' : 'bg-indigo-400/10 text-indigo-400'}`}>{icon}</div><div><h3 className="font-medium">{title}</h3><p className="text-sm text-text-secondary">{subtitle}</p></div></div>
