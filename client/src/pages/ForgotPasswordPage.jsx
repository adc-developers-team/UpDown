    ...
    <div className="min-h-screen bg-chat-bg text-primary flex items-center justify-center p-4 pb-20 pb-20">
      <div className="w-full max-w-md bg-surface rounded-2xl shadow-2 p-8">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <input type="email" placeholder="Enter your email" value={email} onChange={e => setEmail(e.target.value)} required className="w-full bg-bg-input rounded-xl px-4 py-3 outline-none text-primary border border-border-light focus:border-primary" />
          <button type="submit" disabled={loading} className="w-full bg-primary text-primary py-3 rounded-full font-semibold hover:bg-primary-dark transition">{loading ? 'Sending...' : 'Send Reset Link'}</button>
        </form>
