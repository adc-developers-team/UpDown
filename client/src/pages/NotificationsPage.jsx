  ...
  return (
    <div className="min-h-screen bg-chat-bg text-primary flex flex-col pb-20">
      ...
      <header className="flex items-center gap-4 px-4 py-3 bg-dark-blue border-b border-border-light sticky top-0 z-20">
        ...
        {requests.length > 0 && (
          <span className="ml-auto bg-accent text-black text-xs font-bold px-2 py-0.5 rounded-full">
            {requests.length} new
          </span>
        )}
      </header>
                  ...
                  <div className="w-12 h-12 rounded-full bg-accent/20 flex items-center justify-center flex-shrink-0 overflow-hidden">
                    {fromUser.profilePic ? (
                      ...
                      <span className="text-lg font-bold text-accent">
                        {fromUser.fullName?.[0] || fromUser.username?.[0]?.toUpperCase()}
                      </span>
                    )}
                  </div>
                  <div className="flex-1 min-w-0">
                    <h3 className="font-semibold text-sm">{fromUser.fullName || fromUser.username}</h3>
                    <p className="text-xs text-text-secondary truncate">{fromUser.email}</p>
                    <p className="text-xs text-accent mt-0.5">wants to be your friend</p>
                  </div>
