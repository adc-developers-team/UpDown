...
import { FiArrowLeft, FiSend, FiImage, FiUsers } from 'react-icons/fi';
  ...
  return (
    <div className="h-screen flex flex-col bg-chat-bg text-white">
      ...
      <header className="flex items-center gap-4 px-4 py-3 bg-dark-blue border-b border-gray-700">
        ...
        <div className="w-6 h-6 text-gray-400"><FiUsers size={18} /></div>
      </header>

      {/* Messages */}
      <div className="flex-1 p-3 sm:p-4 overflow-y-auto space-y-4">
        {typingUsers.length > 0 && (
          <div className="text-xs text-green-400 mb-2 italic">
          ...
          return (
            <div key={i} className={`flex ${isMine ? 'justify-end' : 'justify-start'}`}>
              <div className={`max-w-[75%] px-4 py-2 rounded-2xl ${
                isMine ? 'bg-light-blue text-white rounded-br-none' : 'bg-gray-700 text-gray-100 rounded-bl-none'
              }`}>
                {!isMine && (
                  <p className="text-xs font-medium text-light-blue mb-0.5">
                    {msg.sender.fullName || msg.sender.username}
                  </p>
                )}
                {msg.image && (
                  <img src={msg.image} className="rounded-lg mb-1 max-w-full" alt="" />
                )}
      ...
      <form onSubmit={handleSend} className="p-3 bg-sidebar-bg border-t border-gray-700 flex items-center gap-2">
        <button
          ...
          className={`w-10 h-10 rounded-full flex items-center justify-center ${isImageMode ? 'bg-light-blue text-white' : 'bg-gray-700 text-gray-400'}`}
        >
          <FiImage size={20} />
        </button>
        {isImageMode ? (
          <input
            ...
            className="flex-1 bg-gray-800 rounded-full px-4 py-2 outline-none text-white placeholder-gray-400 text-sm"
          />
        ) : (
          <input
            type="text"
        ...
        <button
          type="submit"
          className="w-10 h-10 bg-light-blue rounded-full flex items-center justify-center hover:bg-blue-600 transition"
        >
          <FiSend size={18} />
        </button>
      </form>
