...
const { signup, login, verifyEmail, resendVerification, refreshToken, forgotPassword, resetPassword, getSessions, removeSession, logout } = require('../controllers/authController');
...
router.put('/block/:userId', protect, async (req, res) => {
  try {
    ...
    if (!user.blockedUsers.includes(targetId)) {
      ...
      await user.save();
      // Also remove mutual friendship from target
    ...
    res.json({ message: 'Already blocked', blockedUsers: user.blockedUsers });
  } catch (error) { res.status(500).json({ message: error.message }); }
});
...
router.get('/blocked', protect, async (req, res) => {
  try {
    const user = await User.findById(req.user._id).populate('blockedUsers', 'username fullName email profilePic');
